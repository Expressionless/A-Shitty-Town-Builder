...
public class Chunk {
	...
	public Chunk(Biome b, float x, float y, BType type) {
		this.type = type;
	...
	public Tile[][] createChunk() {
		...
		switch (type) {
		case PLAINS:
			sprite_sheet = ResourceLoader.TILE_SHEETS.get("grass");
			break;
		case FOREST:
			sprite_sheet = ResourceLoader.TILE_SHEETS.get("grass");
			break;
		case DEAD_FOREST:
			sprite_sheet = ResourceLoader.TILE_SHEETS.get("sand");
			break;
		default:
			sprite_sheet = ResourceLoader.TILE_SHEETS.get("grass");
			break;
	...
	public void render(Graphics g, int climate) {
		...
		g.draw(bounds);
		g.drawString(type.name(), bounds.getX(), bounds.getY());
		*/
	}
